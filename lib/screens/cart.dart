...
class _CartPageState extends State<CartPage> {
  ...
  loadUserCart() async {
    try {
      ...
      final response = await http.get(
        Uri.parse("${MyConfig.server}/book_bytes/php/load_cart.php?userid=$userid"),
      );
  ...
  _updateCartQuantity(String cartId, String newQuantity) async {
    try {
      await http.post(
        Uri.parse("${MyConfig.server}/book_bytes/php/update_cart.php"),
        body: {
          "cart_id": cartId,
  ...
  _deleteCartItem(Cart cartItem) async {
    try {
      final response = await http.post(
        Uri.parse("${MyConfig.server}/book_bytes/php/delete_cart.php"),
        body: {
